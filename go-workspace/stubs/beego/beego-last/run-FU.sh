...
export CHECK=/data/suz305/go-workspace/my-tools/bin/gochecker
...
function check() {
    ...
    $CHECK -path=${PATH_TO_APP} -include=${APP_URL} -r=true -checker=unlock > ${GOPATH}/results/FU.S.detect.result; echo "${APP} done" &
    popd > /dev/null
