...
# set check path
export CHECK=/data/suz305/go-workspace/my-tools/bin/IF-C3
...
function check() {
    ...
    $CHECK -path=${PATH_TO_APP} -include=${APP_URL} -r > ${GOPATH}/results/IF.detect.result; echo "${APP} done" &
    popd > /dev/null
...
function check_go() {
    ...
    mkdir -p ${GOPATH}/results
    $CHECK_GO -path=${PATH_TO_APP} -include=${APP_URL} -r > ${GOPATH}/results/IF.detect.result; echo "${APP} done" &
...
# stubs/v2ray/v2ray-last/src/v2ray.com/core/
...
# Move the original traefik src under v2 dir
check traefik github.com/containous/traefik/v2   f624449ccbf42c56279c594eadc226fed6583993   
# Move the original gogs src under gogs.io/gogs
