...
function check() {
    ...
    $CHECK -path=${PATH_TO_APP} -include=${APP_URL} -r -race > ${GOPATH}/results/AF.detect.result; echo "${APP} done" &
    popd > /dev/null
...
function check_go() {
    ...
    mkdir -p ${GOPATH}/results
    $CHECK_GO -path=${PATH_TO_APP} -include=${APP_URL} -r -race > ${GOPATH}/results/AF.detect.result; echo "${APP} done" &
