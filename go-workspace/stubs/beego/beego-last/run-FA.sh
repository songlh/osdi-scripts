...
# set check path
export CHECK=/data/suz305/go-workspace/my-tools/bin/FA-C8
...
function check() {
    ...
    $CHECK -path=${PATH_TO_APP} -include=${APP_URL} -r > ${GOPATH}/results/FA.detect.result; echo "${APP} done" &
    popd > /dev/null
...
function check_go() {
    ...
    mkdir -p ${GOPATH}/results
    $CHECK_GO -path=${PATH_TO_APP} -include=${APP_URL} -r > ${GOPATH}/results/FA.detect.result; echo "${APP} done" &
